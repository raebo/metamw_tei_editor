...
const ChooseGbLetterDialog = (props: DefaultDialogProps) => {
  ...
  const fetchLetterAuthors = async (searchValue: string | null) => {
    try {
      const writers: SnippetEntity[] = await EditorUtils.backendService.fetchLetterAuthorsSenders(
        searchValue,
        senderType,
      );
      setLetterAuthors(writers);
    } catch (error) {
      enqueueSnackbar('Error fetching writers ' + MiscUtils.misc.getErrorMessage(error), {
        variant: 'error',
      });
    }
  };
  ...
  const fetchAuthorLetters = async (writer: SnippetEntity, searchValue: string | null) => {
    try {
      ...
      enqueueSnackbar(
        'Error fetching letters for writer ' + MiscUtils.misc.getErrorMessage(error),
        { variant: 'error' },
      );
    }
  };
  ...
  const fetchSenderReceiverLetters = async (searchValue: string | null) => {
    try {
      ...
      enqueueSnackbar(
        'Error fetching sending receiver letters ' + MiscUtils.misc.getErrorMessage(error),
        { variant: 'error' },
      );
    }
  };
  ...
  const handleSubmitButtonClick = async () => {
    try {
      ...
      enqueueSnackbar(
        'Fehler beim Auszeichnen des Briefs' + MiscUtils.misc.getErrorMessage(error),
        { variant: 'error' },
      );
      props.onClose();
    }
      ...
      <Box component="form" sx={{ p: 4, maxWidth: 1200, mx: 'auto' }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
          ...
          <Button
            ...
            disabled={addedLetters.length === null}
          >
