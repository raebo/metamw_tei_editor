...
const ManageTeiHeaderDialog = (props: DefaultDialogProps) => {
  ...
  const [completionState, setCompletionState] = React.useState<CompletionState>({
    ...
    editionValue: 'FMB-C',
    writingPlaceAutoAvailable: false,
    writingPlace: null,
    receiverAutoAvailable: true,
    receivers: [],
    receivingPlaceAutoAvailable: false,
    receivingPlace: null,
    letterLanguage: [],
    editorValue: null,
  ...
  useEffect(() => {
    const saveIsAvailable = (): boolean => {
      ...
      if (
        completionState.firstHeaderContent === null ||
        completionState.sndHeaderContent === null
      ) {
  ...
  const writeHeaderData = (): boolean => {
    ...
    try {
      EditorUtils.teiHeaderContent.setTitleElementHeadlines(
        docData.teiHeader,
        completionState.firstHeaderContent,
        completionState.sndHeaderContent,
      );
      EditorUtils.teiHeaderContent.setPrevNextLetter(
        docData.teiHeader,
        'precursor',
        completionState.prevLetterType,
        completionState.prevLetter,
      );
      EditorUtils.teiHeaderContent.setPrevNextLetter(
        docData.teiHeader,
        'successor',
        completionState.nextLetterType,
        completionState.nextLetter,
      );
      EditorUtils.teiHeaderContent.setWritingPlace(docData.teiHeader, completionState.writingPlace);
      EditorUtils.teiHeaderContent.setReceivingPlace(
        docData.teiHeader,
        completionState.receivingPlace,
      );
      ...
      EditorUtils.teiHeaderContent.setEditorTranskriptorName(
        docData.teiHeader,
        'edition',
        completionState.editorValue,
      );
      EditorUtils.teiHeaderContent.setEditorTranskriptorName(
        docData.teiHeader,
        'transcription',
        completionState.transkriptorValue,
      );
    } catch (error) {
      enqueueSnackbar(
        'Fehler beim Schreiben der TEI-Header-Daten: ' + MiscUtils.misc.getErrorMessage(error),
        { variant: 'error' },
      );
      return false;
    }
  ...
  const submitSaveHandler = async () => {
    if (!docData.xmlDoc || !docData.teiHeader) {
      enqueueSnackbar('Ungültiges XML-Dokument oder kein TEI-Header gefunden', {
        variant: 'error',
      });
      return;
    }
    ...
    try {
      ...
      props.onSave(
        docData.xmlDoc,
        EditorConstants.changeTypes.misc.HEADER_UPDATED,
        'Der HEI-Header wurde aktualisiert.',
        null,
      );
    } catch (error) {
      enqueueSnackbar(
