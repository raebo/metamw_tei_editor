...
const ManageWritingActAuthorWriterDialog = (props: DefaultDialogProps) => {
  const stateActOfWriting = useSelector(
    (state: RootState) => state.editorLetter.letter.actOfWriting,
  );
  ...
  useEffect(() => {
    const extractExistingAuthorsWriters = (xmlDoc: XMLDocument) => {
      try {
        const { authors, writers } = EditorUtils.writingActContent.extractAuthorsWriters(
          xmlDoc,
          stateActOfWriting.orderNumber,
        );
  ...
  const handleSave = async () => {
    ...
    try {
      EditorUtils.writingActContent.setAuthorsWriters(
        xmlDoc,
        stateActOfWriting.orderNumber,
        authors,
        writers,
      ...
      props.onSave(
        xmlDoc,
        EditorConstants.changeTypes.writing_act.MANAGE_AUTHORS_WRITERS,
        'Autoren/Schreiber im Schreibakt wurden aktualisiert.',
        null,
      );
    } catch (error) {
      enqueueSnackbar(MiscUtils.misc.getErrorMessage(error), { variant: 'error' });
  ...
  return (
    <>
      ...
      <Divider />
      <DialogActionButton
        label={'Autoren/Schreiber im Schreibakt Speichern'}
        onClick={handleSave}
        disabled={false}
      />
    </>
