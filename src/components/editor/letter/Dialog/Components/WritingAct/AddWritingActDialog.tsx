...
const AddWritingActDialog = (props: DefaultDialogProps) => {
  ...
  const writeDateToDoc = async () => {
    ...
    try {
      ...
      props.onSave(
        ...
        'Neuer Schreibakt wurde hinzugefügt',
        null,
      );
    } catch (error) {
      enqueueSnackbar('Error durign adding writing act: ' + MiscUtils.misc.getErrorMessage(error), {
        variant: 'error',
      });
  ...
  return (
    <>
      ...
      <Divider />
      <DialogActionButton
        label={'Schreibakt Hinzufügen'}
        onClick={writeDateToDoc}
        disabled={authors.length === null || writers.length === null}
      />
    </>
  );
};
