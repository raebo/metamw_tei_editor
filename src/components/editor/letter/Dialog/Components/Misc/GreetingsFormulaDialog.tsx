...
const GreetingsFormulaDialog = (props: GreetingsFormulaProps) => {
	...
	const [formData, setFormData] = React.useState<GreetingFormData | null>({ greetingIsNew: true, greetingPosition: 'center', greetingText: null});
					...
					<div className="form-item form-item--key">
						<FormControl variant="filled" sx={{m: 1, minWidth: 120, width: '100%'}}>
							...
							<Select
								...
								onChange={(event) => updateFormData({ greetingPosition: event.target.value as 'center' | 'left' | 'right' })}
								>
								{[
