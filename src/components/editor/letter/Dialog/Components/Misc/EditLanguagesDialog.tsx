...
const EditLanguagesDialog = (props: DefaultDialogProps) => {
  ...
  useEffect(() => {
    const assignedLanguages: LanguageOption[] =
      EditorUtils.teiHeaderContent.extractLanguages(teiHeader);
  ...
  const handleSave = async () => {
    ...
    props.onSave(
      xmlDoc,
      EditorConstants.changeTypes.misc.HEADER_LANGUAGES_UPDATED,
      'Sprachen im Header wurden aktualisiert',
      null,
    );
  };
  ...
  return (
    <>
      ...
      <DialogActionButton
        label={'Sprachen Speichern'}
        onClick={handleSave}
        disabled={letterLanguages.length == 0}
      />
    </>
  );
};
