...
const ManageTeiHeaderReceiverDialog = (props: DefaultDialogProps) => {
  ...
  useEffect(() => {
    ...
    if (docData.teiHeader) {
      try {
        setReceivers(EditorUtils.teiHeaderContent.extractReceivers(docData.teiHeader));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      props.onSave(
        ...
        'Die Empfänger des Briefes wurden aktualisiert.',
        null,
      );
    } catch (error) {
      enqueueSnackbar(
  ...
  return (
    <>
      ...
      <Divider />
      <DialogActionButton
        label={'Empfänger im Header Speichern'}
        onClick={handleSave}
        disabled={false}
      />
    </>
