...
const blankPlaceFormData: PlaceFormData = {
  key: '',
  name: '',
  placeType: '',
...
const EntityPlaceContainer = (props: EditorContainerProps) => {
  ...
  useEffect(() => {
    ...
    const firstRadio = radioGroupRef.current.querySelector<HTMLInputElement>('input[type=radio]');
    firstRadio?.focus();
  }, []);
  ...
  useEffect(() => {
    ...
    if (kindOptions.length === 0) {
      void fetchKindOptions();
    }
  }, [countryOptions.length, kindOptions.length]);

  const fetchAndSetNewKey = (entityType: SelectTypeOption) => {
    if (entityType === null) {
      return;
    }
    fetchEntityKey(entityType)
      .then((key: string) => {
      ...
      <Box component="form" sx={{ p: 4, maxWidth: 600, mx: 'auto' }}>
        ...
        <FormControl component="fieldset" sx={{ mb: 3 }}>
          <RadioGroup
            ...
            onChange={(e) => handleSelectActionOption(e.target.value as SelectActionOption)}
          >
            <FormControlLabel
          ...
          <Grid size={12}>
            {(selectedKindOption === 'EXISTING_ENTRY' || selectedKindOption === null) && (
              <PlaceKindAutocomplete
                ...
                selectedOption={
                  placeFormData.kind !== null && placeFormData.kind !== undefined
                    ? { label: placeFormData.kind, value: placeFormData.kind }
                    : { label: '', value: '' }
                }
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
          <Button
            variant="contained"
            color="primary"
            onClick={buttonAddNewPlaceEntry}
