...
const EntityProtagCreationContainer = (props: EditorContainerProps) => {
  ...
  const fetchAndSetNewProtagCreationKey = () => {
    fetchEntityKey(EntityType.PROTAG_CREATION).then((key: string) => {
  ...
  const removeExistingEntry = (clickedEntry: MarkupProtagCreationData) => {
    setAddedProtagCreations((prevEntries) => prevEntries.filter((entry: MarkupProtagCreationData) => {
  ...
  return (
    <>
      <Box
        component="form"
        sx={{ p: 4, maxWidth: 600, px: "5px", mx: 'auto' }}
      >
        ...
        <FormControl component="fieldset" sx={{ mb: 3 }}>
          <RadioGroup row value={selectedProtagCreationOption} onChange={(e) => (handleProtagOptionChange(e.target.value as SelectActionProtagCreationOption))}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
          <Button variant="contained" color="primary" onClick={buttonAddNewProtagEntry} disabled={!isValidFormData()} startIcon={<AddIcon />}>
            FMB-Werk Hinzufügen
          </Button>
