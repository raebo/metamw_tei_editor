...
export const miscContentCheck = {
  writeAddressEntry: (
    ...
    contentAddressEntry: ContentAddressEntry | null,
  ): void => {
    ...
    if (contentAddressEntry.hasParagraph) {
      ...
      if (
        contentAddressEntry.paragraph &&
        validParagraphClasses.includes(contentAddressEntry.paragraph)
      ) {
  ...
  existingAddressEntry: (
    ...
    const contentAddressEntry: ContentAddressEntry = {
      isNewEntry: true,
      deleteAddress: false,
      type: addressType,
      hasHead: false,
    ...
    if (headMarkup) {
      ...
      addrLinesMarkup = EditorUtils.xmlCheck.queryPath(
        addressContainer,
        'head > address > addrLine',
      );
    }
    ...
    if (pMarkup) {
      ...
      if (style && ['PARAGRAPH_LEFT', 'PARAGRAPH_RIGHT', 'PARAGRAPH_CENTER'].includes(style)) {
        contentAddressEntry.paragraph = style.toUpperCase() as
          | 'PARAGRAPH_LEFT'
          | 'PARAGRAPH_RIGHT'
          | 'PARAGRAPH_CENTER';
      }
  ...
  nodeRightClickMenu: (node: Node, menuType: 'MOVE_UP' | 'MOVE_DOWN' | 'SALUTE'): string => {
    switch (menuType) {
      case 'MOVE_UP':
        return EditorUtils.writingActContent.currentActIsMovableUp(node as Element)
          ? EditorConstants.menuItemTypes.WRITING_ACT.MOVE_UP
          : EditorConstants.menuItemTypes.WRITING_ACT.LABEL_MOVE_UP;
      case 'MOVE_DOWN':
        return EditorUtils.writingActContent.currenActIsMovableDown(node as Element)
      ...
      case 'SALUTE':
        if (node.nodeName.toLowerCase() === 'salute') {
          return EditorConstants.menuItemTypes.WRITING_ACT.MANAGE_GREETINGS_FORMULA;
        } else {
          return 'UNLNOWN_MENU_TYPE';
        }
    }

    throw new Error(`Menu type for ${menuType} not found`);
  },
};
